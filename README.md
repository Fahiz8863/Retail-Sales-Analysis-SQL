...
FROM retail_sales;
```
- **Total Sales**: Find out the total sales in 2022& 2023.
```sql
SELECT 
...
3. **Write a SQL query to calculate the total sales(total_sale), total order count and total quantity ordered for each category.**:
```sql
SELECT 
	category, 
	SUM(total_sale) as total_sales,
